// contexts/ChatContext.js - Updated to use P2PRoomContext
...
import { useP2PRoom } from './P2PRoomContext.js';
...
export const ChatProvider = ({ children, onBack }) => {
  ...
  const handleInputSubmit = useCallback((localInputVal) => {
    ...
    if (localInputVal.trim() === '/clear') {
      setInputValue('');
      // Clearing history isn't implemented in P2PRoomContext
      // We would need to add a clearMessages function there
      return true;
    }
    ...
    // Add this to the handleInputSubmit function in ChatContext.js
      ...
      if (username) {
        ...
        if (success) {
          ...
          if (activeRoomId) {
            ...
            sendMessage(activeRoomId, message.content);
          }
        }
      }
      return true;
    ...
    if (localInputVal.trim() === '/invite' && activeRoomId) {
      const inviteCode = createInviteCode(activeRoomId);
      if (inviteCode) {
        // Display a message to the user that the invite was copied
        const message = {
          id: `system-${Date.now()}`,
          content: `Invite code copied to clipboard! Others can join using "/join ${inviteCode}"`,
          sender: 'System',
          timestamp: Date.now(),
          system: true
        };
    ...
    if (focusedPanel === 'rooms' && inputMode) {
      ...
      if (activeRoomId) {
        ...
        // Then send the message using P2PRoomContext
  ...
  const handleFileSelect = useCallback((files) => {
    ...
    // Send the message using P2PRoomContext
    if (activeRoomId) {
